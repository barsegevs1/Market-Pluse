...
const INDICES = [
  { key: 'SP500',   symbol: 'SPY',     label: 'S&P 500',       color: '#4ade80' },
  { key: 'NASDAQ',  symbol: 'QQQ',     label: 'NASDAQ',        color: '#60a5fa' },
  { key: 'TA35',    symbol: 'EIS',  label: 'TA-35',         color: '#f59e0b' },
  { key: 'STOXX',   symbol: 'FEZ', label: 'Euro Stoxx 50', color: '#a78bfa' },
  { key: 'NIKKEI',  symbol: 'EWJ',     label: 'Nikkei 225',    color: '#f87171' },
...
const fmtCap = (n: number) => n >= 1e12 ? `$${fmt(n/1e12,2)}T` : n >= 1e9 ? `$${fmt(n/1e9,1)}B` : n > 0 ? `$${fmt(n/1e6,0)}M` : '—'
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const stockData = await fetchYahoo(TICKERS.map(t => t.ticker), '1M')
      const rows: StockRow[] = TICKERS.map(t => {
        ...
        return { ticker:t.ticker, name:t.name, sector:t.sector, index:t.index,
          marketCap:meta.marketCap||0, pe:meta.trailingPE||0,
          price, change1d, change5d, change21d, high52w, dropFromHigh,
          delta1d:0, delta5d:0, delta21d:0, score:0 }
      }).filter(Boolean) as StockRow[]

      const sectors = [...new Set(rows.map(r => r.sector))]
      sectors.forEach(sec => {
        const g = rows.filter(r => r.sector === sec)
        const a1=g.reduce((s,r)=>s+r.change1d,0)/g.length
        const a5=g.reduce((s,r)=>s+r.change5d,0)/g.length
        const a21=g.reduce((s,r)=>s+r.change21d,0)/g.length
        g.forEach(r => { r.delta1d=r.change1d-a1; r.delta5d=r.change5d-a5; r.delta21d=r.change21d-a21 })
      })

      const all1d=rows.map(r=>r.delta1d), all5d=rows.map(r=>r.delta5d)
      const all21d=rows.map(r=>r.delta21d), allDrop=rows.map(r=>r.dropFromHigh)
      rows.forEach(r => {
        r.score = parseFloat(((
          percentileRank(all1d,r.delta1d)+percentileRank(all5d,r.delta5d)+
          percentileRank(all21d,r.delta21d)+percentileRank(allDrop,r.dropFromHigh)
        )/4).toFixed(1))
      })
  ...
  const S: Record<string,any> = {
    ...
    hdr:   { display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 20px', borderBottom:'1px solid #21262d' },
    wrap:  { padding:'20px', maxWidth:1400, margin:'0 auto' },
    card:  { background:'#161b22', border:'1px solid #21262d', borderRadius:12, padding:20, marginBottom:24 },
    pill:  (active:boolean,color:string)=>({ display:'flex',alignItems:'center',gap:6,padding:'4px 12px',borderRadius:20,border:`1px solid ${active?color:'#30363d'}`,background:active?`${color}22`:'transparent',color:active?color:'#8b949e',cursor:'pointer',fontSize:13 }),
    rBtn:  (active:boolean)=>({ padding:'4px 10px',borderRadius:6,border:'none',background:active?'#238636':'transparent',color:active?'#fff':'#8b949e',cursor:'pointer',fontSize:13 }),
    mBtn:  (active:boolean)=>({ padding:'4px 12px',borderRadius:6,border:'none',background:active?'#21262d':'transparent',color:active?'#e6edf3':'#8b949e',cursor:'pointer',fontSize:13 }),
    th:    { padding:'8px 12px', textAlign:'left' as const, color:'#8b949e', fontWeight:500, cursor:'pointer', whiteSpace:'nowrap' as const, userSelect:'none' as const },
  ...
  return (
    <div style={S.page}>
        ...
        <div style={S.card}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
            <div>
              ...
              <p style={{fontSize:12,color:'#8b949e',marginTop:2}}>{chartMode==='relative'?'Normalized to 0% at start of range.':'Actual values.'}</p>
            </div>
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              <div style={{display:'flex',background:'#0d1117',borderRadius:8,padding:2}}>
                {(['relative','price'] as const).map(m=>(
                  <button key={m} onClick={()=>setChartMode(m)} style={S.mBtn(chartMode===m)}>{m==='relative'?'Relative %':'Price'}</button>
                ))}
              </div>
              <div style={{display:'flex',background:'#0d1117',borderRadius:8,padding:2}}>
                {RANGES.map(r=><button key={r} onClick={()=>setRange(r)} style={S.rBtn(range===r)}>{r}</button>)}
              </div>
            </div>
          </div>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:8,marginBottom:16}}>
            {INDICES.map(idx=>(
              <button key={idx.key} onClick={()=>toggleIdx(idx.key)} style={S.pill(activeIdx.has(idx.key),idx.color)}>
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData}>
              ...
              <Tooltip contentStyle={{background:'#161b22',border:'1px solid #30363d',borderRadius:8,fontSize:12}} labelStyle={{color:'#8b949e'}}
            ...
            {INDICES.filter(i=>activeIdx.has(i.key)).map(idx=>{
              ...
              return (
                <div key={idx.key} style={{display:'flex',alignItems:'center',gap:6,fontSize:13}}>
                  ...
                  {last==null?<span style={{color:'#f87171'}}>error</span>:<span style={{color:last>=0?'#4ade80':'#f87171'}}>{fmtPct(last)}</span>}
                </div>
              )
            })}
          </div>
              ...
              <thead>
                <tr style={{borderBottom:'1px solid #21262d'}}>
                  {([['ticker','TICKER'],['name','NAME'],['index','INDEX'],['sector','SECTOR'],
                     ['marketCap','MKT CAP'],['pe','P/E'],['price','PRICE'],['score','SCORE'],
                     ['delta1d','Δ1D vs SEC'],['delta5d','Δ5D vs SEC'],['delta21d','Δ21D vs SEC'],
                     ['dropFromHigh','52W HIGH']] as [keyof StockRow,string][]).map(([col,label])=>(
                    <th key={col} onClick={()=>handleSort(col)} style={S.th}>
              ...
              <tbody>
                {sorted.map((row,i)=>(
                  <tr key={row.ticker} style={{borderBottom:'1px solid #21262d',background:i%2===0?'transparent':'#0d111722'}}>
                    <td style={{...S.td,fontWeight:700}}>{row.ticker}</td>
                    <td style={{...S.td,color:'#8b949e'}}>{row.name}</td>
                    <td style={S.td}>
                      <span style={{padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:600,
                        background:row.index==='NASDAQ'?'#1d4ed822':row.index==='NYSE'?'#7e22ce22':'#d9770622',
                        color:row.index==='NASDAQ'?'#60a5fa':row.index==='NYSE'?'#a78bfa':'#fb923c'}}>
                        {row.index}
                      </span>
                    </td>
                    <td style={{...S.td,color:'#8b949e'}}>{row.sector}</td>
                    <td style={S.td}>{fmtCap(row.marketCap)}</td>
                    <td style={S.td}>{row.pe?fmt(row.pe,1):'—'}</td>
                    <td style={S.td}>${fmt(row.price)}</td>
                    <td style={S.td}>
                      <span style={{padding:'3px 10px',borderRadius:20,fontWeight:700,background:`${scoreColor(row.score)}22`,color:scoreColor(row.score)}}>
                        {row.score}
                      </span>
                    </td>
                    <td style={{...S.td,color:row.delta1d<0?'#f87171':'#4ade80'}}>{fmtPct(row.delta1d)}</td>
                    <td style={{...S.td,color:row.delta5d<0?'#f87171':'#4ade80'}}>{fmtPct(row.delta5d)}</td>
                    <td style={{...S.td,color:row.delta21d<0?'#f87171':'#4ade80'}}>{fmtPct(row.delta21d)}</td>
                    <td style={{...S.td,color:row.dropFromHigh<-15?'#f87171':'#8b949e'}}>{fmtPct(row.dropFromHigh)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@media(max-width:768px){table{font-size:11px}th,td{padding:6px 8px!important}}`}</style>
    </div>
  )
}
